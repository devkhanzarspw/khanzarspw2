...
public final class KeuanganPiutangObatBelumLunas extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        BtnAll2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/female-user-help24.png"))); // NOI18N
        BtnAll2.setMnemonic('M');
        BtnAll2.setText("Karywan");
        BtnAll2.setToolTipText("Alt+M");
        BtnAll2.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        BtnAll2.setName("BtnAll2"); // NOI18N
        BtnAll2.setPreferredSize(new java.awt.Dimension(90, 23));
        ...
        BtnAll3.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/family24.png"))); // NOI18N
        BtnAll3.setMnemonic('M');
        BtnAll3.setText("Kronis");
        BtnAll3.setToolTipText("Alt+M");
        BtnAll3.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        BtnAll3.setName("BtnAll3"); // NOI18N
        BtnAll3.setPreferredSize(new java.awt.Dimension(95, 23));
...
private void MnDetailPiutangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDetailPiutangActionPerformed
    ...
    */
    ...
    private void tampil2(){
        Valid.tabelKosong(tabMode);
        tgl1 = Valid.SetTgl(Tanggal1.getSelectedItem().toString());
        tgl2 = Valid.SetTgl(Tanggal2.getSelectedItem().toString());
        try{
            sisapiutang=0;
            ps=koneksi.prepareStatement(
                    "select piutang.nota_piutang,piutang.tgl_piutang,piutang.no_rkm_medis,piutang.nm_pasien,piutang.catatan,piutang.ongkir,piutang.uangmuka,piutang.sisapiutang,"+
                    "piutang.tgltempo,(select ifnull(SUM(bayar_piutang.besar_cicilan),0) from bayar_piutang where bayar_piutang.no_rawat=piutang.nota_piutang) as cicilan  "+
                    "from piutang WHERE piutang.tgl_piutang BETWEEN ? and ? and piutang.nota_piutang LIKE \"%HK%\" having piutang.sisapiutang-cicilan>0 order by piutang.tgl_piutang");
            try {
                if(TCari.getText().trim().equals("")){
                    ps.setString(1,tgl1);
                    ps.setString(2,tgl2);
    ...
    private void tampil3(){
        ...
        try{
            ...
            ps=koneksi.prepareStatement(
                    "select piutang.nota_piutang,piutang.tgl_piutang,piutang.no_rkm_medis,piutang.nm_pasien,piutang.catatan,piutang.ongkir,piutang.uangmuka,piutang.sisapiutang,"+
                    "piutang.tgltempo,(select ifnull(SUM(bayar_piutang.besar_cicilan),0) from bayar_piutang where bayar_piutang.no_rawat=piutang.nota_piutang) as cicilan  "+
                    "from piutang WHERE piutang.tgl_piutang BETWEEN ? and ? and piutang.nota_piutang NOT LIKE \"%HK%\" having piutang.sisapiutang-cicilan>0 order by piutang.tgl_piutang");
            try {
                if(TCari.getText().trim().equals("")){
                    ps.setString(1,tgl1);
                    ps.setString(2,tgl2);
