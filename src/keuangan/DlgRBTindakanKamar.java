...
public class DlgRBTindakanKamar extends javax.swing.JDialog {
    ...
    private PreparedStatement psbangsal,pspasien,psobat,psobatlangsung,psperiksalab,psdetailperiksalab,psretur,
            psoperasi,psranapdr,psranapdrpr,psranappr,pstambahan,pspotongan,psregistrasi,psradiologi,pskamar, psbaru1, psbaru2;
    private ResultSet rsbangsal,rspasien,rs, rs1,rs2,rs3; 
    private Dimension screen=Toolkit.getDefaultToolkit().getScreenSize();
    private DlgCariBangsal bangsal=new DlgCariBangsal(null,false);
    private DlgCariCaraBayar penjab=new DlgCariCaraBayar(null,false);
    private int i=0,a=0;
    private double obat=0,retur=0,obatlangsung=0,laborat=0,operasi=0,radiologi=0,jm=0,jm2=0,jm3=0,ttlbiaya=0,detailobat=0,detailobatlangsung=0,ttlobat=0,ttlobatlangsung=0,ttllaborat=0,ttljm=0,
    ...
    public DlgRBTindakanKamar(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"No.","Bangsal","Tgl.Masuk","Tgl.Keluar","Cara Bayar","Obt+Emb+Tsl-retur","Laborat","Paket Tindakan","Tambahan","Potongan","Registrasi","Radiologi","Kamar","Total"};
                ...
                while(rsbangsal.next()){
                    ...
                    try {
                        ...
                        while(rspasien.next()){
                            ...
                            obat=0;
                            psobat=koneksi.prepareStatement("select sum(dpo.total)-(SELECT IFNULL(SUM(aa.subtotal),0) FROM detreturjual aa\n" +
                            "WHERE SUBSTRING(aa.no_retur_jual,1,17) = dpo.no_rawat) AS beriminret\n" +
...
//                            psranapdrpr=koneksi.prepareStatement("select sum(biaya_rawat) from rawat_inap_drpr where no_rawat=?");
                            psranapdrpr=koneksi.prepareStatement("SELECT (SELECT SUM(rjd.biaya_rawat) FROM rawat_jl_dr rjd WHERE no_rawat=? )+\n" +
                            "(SELECT SUM(rjp.biaya_rawat) FROM rawat_jl_pr rjp WHERE no_rawat=? ) AS igd");
                            try {
                                psranapdrpr.setString(1,rspasien.getString("no_rawat"));
                                psranapdrpr.setString(2,rspasien.getString("no_rawat"));
                            ...
                            "biayasewaok+akomodasi+bagian_rs+biaya_omloop+biaya_omloop2+biaya_omloop3+biayasarpras)+\n" +
                            "(SELECT IFNULL(SUM(boo.hargasatuan*boo.jumlah),0) FROM beri_obat_operasi boo WHERE boo.no_rawat =?)\n" +
                            "from operasi where no_rawat=?");
                             try {
                                psoperasi.setString(1,rspasien.getString("no_rawat"));
                                psoperasi.setString(2,rspasien.getString("no_rawat"));
                            ...
                            tabMode.addRow(new Object[]{
                                ...
                                rspasien.getString("tgl_masuk"),
                                rspasien.getString("tgl_keluar"),
                                rspasien.getString("png_jawab"),
                                Valid.SetAngka(obat+obatlangsung),
                                Valid.SetAngka(laborat+detaillaborat),
                                Valid.SetAngka(jm+jm2+jm3+operasi),
                                Valid.SetAngka(tambahan),
                                Valid.SetAngka(potongan),
                                Valid.SetAngka(registrasi),
                                Valid.SetAngka(radiologi),
                                Valid.SetAngka(kamar),
                            ...
                            psbaru2=koneksi.prepareStatement("SELECT aa.no_rawat, \"Operator 1\" AS sbg, dr.nm_dokter,\"-\", aa.biayaoperator1 from operasi aa\n" +
                            "JOIN dokter dr ON dr.kd_dokter = aa.operator1 where no_rawat=?\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Operator 2\", dr.nm_dokter,\"-\", aa.biayaoperator2 from operasi aa\n" +
                            "JOIN dokter dr ON dr.kd_dokter = aa.operator2 where no_rawat=? AND aa.operator2 != \"-\"\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Anestesi\", dr.nm_dokter,\"-\", aa.biayadokter_anestesi\n" +
                            "from operasi aa JOIN dokter dr ON dr.kd_dokter = aa.dokter_anestesi where no_rawat=?\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Ass Operator\",\"-\",\"-\", aa.biayaasisten_operator1\n" +
                            "from operasi aa  where no_rawat=?\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Ass Anestesi\",\"-\",\"-\", aa.biayaasisten_anestesi\n" +
                            "from operasi aa  where no_rawat=?\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Instrumen\",\"-\",\"-\", aa.biayainstrumen\n" +
                            "from operasi aa  where no_rawat=?\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, \"Sewa OK\",\"-\",\"-\", aa.biayasewaok\n" +
                            "from operasi aa  where no_rawat=?\n" +
                            "UNION \n" +
                            "SELECT boo.no_rawat, \"BHP\",obo.nm_obat, COUNT(boo.kd_obat),SUM(boo.hargasatuan*jumlah)\n" +
                            "FROM beri_obat_operasi boo\n" +
                            "JOIN obatbhp_ok obo ON obo.kd_obat = boo.kd_obat\n" +
                            "WHERE boo.no_rawat=?\n" +
                            "GROUP BY boo.kd_obat");
                            try {
                                psbaru2.setString(1,rspasien.getString(1));
                                psbaru2.setString(2,rspasien.getString(1));
                                psbaru2.setString(3,rspasien.getString(1));
                                psbaru2.setString(4,rspasien.getString(1));
                                psbaru2.setString(5,rspasien.getString(1));
                                psbaru2.setString(6,rspasien.getString(1));
                                psbaru2.setString(7,rspasien.getString(1));
                                psbaru2.setString(8,rspasien.getString(1));
                                rs2=psbaru2.executeQuery();
                                if(rs2.isBeforeFirst()){
                                    tabMode.addRow(new Object[]{
                                            "", "","","","","","Operasi","Dokter","Jml","Biaya","","","","",
                                    });
                                }
                                while (rs2.next()) {                                    
                                    tabMode.addRow(new Object[]{
                                        "", "","","","","",rs2.getString(2),rs2.getString(3),rs2.getString(4),Valid.SetAngka(rs2.getDouble(5)),"","","","",
                                    });
                                }
                            } catch (Exception e) {
                                System.out.println("Notif Jm : "+e);
                            ...
                            psbaru1=koneksi.prepareStatement("SELECT \"Norw\",\"Ranap\",\"Dokter\",\"\",\"\"\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, SUBSTRING(jpi.nm_perawatan,1,24) ,dr.nm_dokter,COUNT(aa.kd_jenis_prw), SUM(aa.biaya_rawat)\n" +
                            "FROM rawat_inap_dr aa\n" +
                            "JOIN dokter dr ON dr.kd_dokter = aa.kd_dokter\n" +
                            "JOIN jns_perawatan_inap jpi ON jpi.kd_jenis_prw = aa.kd_jenis_prw\n" +
                            "WHERE aa.no_rawat =?\n" +
                            "GROUP BY aa.kd_jenis_prw, aa.kd_dokter\n" +
                            "UNION\n" +
                            "SELECT rip.no_rawat, \"Tindakan Ranap\",\"-\",\"-\",SUM(rip.biaya_rawat)\n" +
                            "FROM rawat_inap_pr rip\n" +
                            "WHERE rip.no_rawat =?\n" +
                            "GROUP BY rip.no_rawat\n" +
                            "UNION\n" +
                            "SELECT \"Norw\",\"IGD\",\"Dokter\",\"\",\"\"\n" +
                            "UNION\n" +
                            "SELECT aa.no_rawat, jpi.nm_perawatan ,dr.nm_dokter,COUNT(aa.kd_jenis_prw), SUM(aa.biaya_rawat)\n" +
                            "FROM rawat_jl_dr aa\n" +
                            "JOIN dokter dr ON dr.kd_dokter = aa.kd_dokter\n" +
                            "JOIN jns_perawatan jpi ON jpi.kd_jenis_prw = aa.kd_jenis_prw\n" +
                            "WHERE aa.no_rawat =?\n" +
                            "GROUP BY aa.kd_jenis_prw, aa.kd_dokter\n" +
                            "UNION\n" +
                            "SELECT rjp.no_rawat, \"Tindakan IGD\",\"-\",\"-\",SUM(rjp.biaya_rawat)\n" +
                            "FROM rawat_jl_pr rjp\n" +
                            "WHERE rjp.no_rawat =?\n" +
                            "GROUP BY rjp.no_rawat");
                            try {
                                psbaru1.setString(1,rspasien.getString(1));
                                psbaru1.setString(2,rspasien.getString(1));
                                psbaru1.setString(3,rspasien.getString(1));
                                psbaru1.setString(4,rspasien.getString(1));
                                rs1=psbaru1.executeQuery();
                                while (rs1.next()) {                                    
                                    tabMode.addRow(new Object[]{
                                        "", "","","","","",rs1.getString(2),rs1.getString(3),rs1.getString(4),Valid.SetAngka(rs1.getDouble(5)),"","","","",
                                    });
                                }
                            }catch (Exception e) {
                                System.out.println("Notif Jm : "+e);
